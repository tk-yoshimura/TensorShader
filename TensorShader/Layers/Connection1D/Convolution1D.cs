...
namespace TensorShader.Layers {
    ...
    public class Convolution1D : Layer {
        ...
        public Convolution1D(int inchannels, int outchannels, int kwidth, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel1D(inchannels, outchannels, kwidth)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = Convolution1D(x, W, Stride);
