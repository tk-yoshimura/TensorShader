...
namespace TensorShader.Layers {
    ...
    public class Deconvolution1D : Layer {
        ...
        public Deconvolution1D(int inchannels, int outchannels, int kwidth, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel1D(outchannels, inchannels, kwidth)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            Field y = Deconvolution1D(x, W, Stride);
