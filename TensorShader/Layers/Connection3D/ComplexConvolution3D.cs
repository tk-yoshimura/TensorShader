...
namespace TensorShader.Layers {
    ...
    public class ComplexConvolution3D : Layer {
        ...
        public ComplexConvolution3D(int inchannels, int outchannels, int kwidth, int kheight, int kdepth, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                Shape.Kernel3D(inchannels, outchannels / 2, kwidth, kheight, kdepth),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = ComplexConvolution3D(x, W);
