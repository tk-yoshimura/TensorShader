...
namespace TensorShader.Layers {
    ...
    public class QuaternionConvolution3D : Layer {
        /// <summary>重み</summary>
        ...
        public QuaternionConvolution3D(int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel3D(inchannels, outchannels / 4, kwidth, kheight, kdepth)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = QuaternionConvolution3D(x, W, Stride);
