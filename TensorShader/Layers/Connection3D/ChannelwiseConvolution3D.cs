...
namespace TensorShader.Layers {
    ...
    public class ChannelwiseConvolution3D : Layer {
        /// <summary>重み</summary>
        ...
        public ChannelwiseConvolution3D(int channels, int kwidth, int kheight, int kdepth, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel3D(channels, 1, kwidth, kheight, kdepth)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = ChannelwiseConvolution3D(x, W, Stride);
