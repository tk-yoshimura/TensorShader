...
namespace TensorShader.Layers {
    ...
    public class ComplexDeconvolution2D : Layer {
        /// <summary>重み</summary>
        ...
        public ComplexDeconvolution2D(int inchannels, int outchannels, int kwidth, int kheight, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel2D(outchannels, inchannels / 2, kwidth, kheight)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            Field y = ComplexDeconvolution2D(x, W, Stride);
