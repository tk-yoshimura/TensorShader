...
namespace TensorShader.Layers {
    ...
    public class ComplexConvolution2D : Layer {
        /// <summary>重み</summary>
        ...
        public ComplexConvolution2D(int inchannels, int outchannels, int kwidth, int kheight, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel2D(inchannels, outchannels / 2, kwidth, kheight)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = ComplexConvolution2D(x, W, Stride);
