...
namespace TensorShader.Layers {
    ...
    public class QuaternionConvolution2D : Layer {
        ...
        public QuaternionConvolution2D(int inchannels, int outchannels, int kwidth, int kheight, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                Shape.Kernel2D(inchannels, outchannels / 4, kwidth, kheight),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = QuaternionConvolution2D(x, W);
