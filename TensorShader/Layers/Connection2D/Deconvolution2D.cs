...
namespace TensorShader.Layers {
    ...
    public class Deconvolution2D : Layer {
        /// <summary>重み</summary>
        ...
        public Deconvolution2D(int inchannels, int outchannels, int kwidth, int kheight, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel2D(outchannels, inchannels, kwidth, kheight)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            Field y = Deconvolution2D(x, W, Stride);
