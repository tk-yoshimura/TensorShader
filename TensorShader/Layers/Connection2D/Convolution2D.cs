...
namespace TensorShader.Layers {
    ...
    public class Convolution2D : Layer {
        ...
        public Convolution2D(int inchannels, int outchannels, int kwidth, int kheight, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel2D(inchannels, outchannels, kwidth, kheight)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            ...
            Field y = Convolution2D(x, W, Stride);
