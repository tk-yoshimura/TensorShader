...
namespace TensorShader.Layers {
    ...
    public class TrivectorDeconvolution2D : Layer {
        /// <summary>重み</summary>
        ...
        public TrivectorDeconvolution2D(int inchannels, int outchannels, int kwidth, int kheight, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel2D(outchannels / 3 * 4, inchannels / 3, kwidth, kheight)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            Field y = TrivectorDeconvolution2D(x, W, Stride);
