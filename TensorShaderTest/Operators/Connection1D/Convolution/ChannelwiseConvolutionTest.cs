...
namespace TensorShaderTest.Operators.Connection1D {
    ...
    public class ChannelwiseConvolutionTest {
                    ...
                    foreach (int kwidth in new int[] { 1, 3, 5 }) {
                        foreach (int stride in new int[] { 1, 2, 3 }) {
                            foreach (int inwidth in new int[] { 8, 9, 13, 17 }) {
                                ...
                                ChannelwiseConvolution ope = new ChannelwiseConvolution(inwidth, channels, kwidth, stride, batch);
                                ...
                                AssertError.Tolerance(y_expect, y_actual, 1e-7f, 1e-5f, ref max_err, $"mismatch value {channels},{kwidth},{stride},{inwidth},{batch}");
        ...
        public void SpeedTest() {
            ...
            ChannelwiseConvolution ope = new ChannelwiseConvolution(inwidth, channels, ksize, stride);
                ...
                for (int th = 0; th < batch; th++) {
                    for (int ox = 0; ox < outw; ox++) {
                        for (int ch = 0; ch < channels; ch++) {
                            y[ch, ox, th] += x[ch, kx + ox * stride, th] * w[ch, 0, kx];
                        }
                    }
                }
            }
