...
namespace TensorShaderTest.Layers {
    ...
    public class ComplexDeconvolution3DTest {
        ...
        public void ExecuteTest() {
            int inchannels = 4, outchannels = 6, inwidth = 13, inheight = 17, indepth = 19, kwidth = 3, kheight = 5, kdepth = 7, batch = 7;

            VariableField x = new Tensor(Shape.Map3D(inchannels, inwidth, inheight, indepth, batch));

            Layer layer = new ComplexDeconvolution3D(inchannels, outchannels, kwidth, kheight, kdepth, use_bias: true, pad_mode: PaddingMode.Edge, "conv");
