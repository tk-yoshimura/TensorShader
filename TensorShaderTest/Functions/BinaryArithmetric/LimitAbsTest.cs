    ...
    public class LimitAbsTest {
        ...
        public void ExecuteTest() {
...

            {
                ...
                AssertError.Tolerance(idxes.Select((idx) => Math.Min(Math.Max(x1[idx], -x2[idx]), x2[idx])).ToArray(), o.State, 1e-7f, 1e-5f);
            }

            {
                ...
                AssertError.Tolerance(idxes.Select((idx) => Math.Min(Math.Max(x1[idx], -x1[idx]), x1[idx])).ToArray(), o.State, 1e-7f, 1e-5f);
            }
