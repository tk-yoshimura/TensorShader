...
namespace TensorShaderCudaBackend.Cudnn {
    ...
    public class FilterDescriptor : IDisposable {
        ...
        /// <param name="h">高さ</param>
        /// <param name="w">幅</param>
        public FilterDescriptor(TensorFormat format, DataType dtype, int k, int c, int d, int h, int w) {
            ptr = API.Cudnn.TensorDescriptor.Create();
            API.Cudnn.FilterDescriptor.Set5d(ptr, format, dtype, k, c, d, h, w);
        }
