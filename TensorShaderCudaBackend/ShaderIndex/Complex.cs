...
namespace TensorShaderCudaBackend {
    ...
    public static class Complex {
        ...
        public static void Dense(uint inchannels, uint outchannels,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Dense(inchannels, outchannels, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Dense(inchannels, outchannels, gradmode));
                }
            }
        ...
        public static void TransposeDense(uint inchannels, uint outchannels,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.TransposeDense(inchannels, outchannels, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.TransposeDense(inchannels, outchannels, gradmode));
                }
            }
        ...
        public static void KernelProductDense(uint inchannels, uint outchannels,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.KernelProductDense(inchannels, outchannels, transpose));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.KernelProductDense(inchannels, outchannels, transpose));
                }
            }
        ...
        public static void Convolution1D(uint inchannels, uint outchannels, uint inwidth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Convolution1D(inchannels, outchannels, kwidth, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Convolution1D(inchannels, outchannels, kwidth, gradmode));
                }
            }
        ...
        public static void Deconvolution1D(uint inchannels, uint outchannels, uint inwidth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Deconvolution1D(inchannels, outchannels, kwidth, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Deconvolution1D(inchannels, outchannels, kwidth, gradmode));
                }
            }
        ...
        public static void KernelProduct1D(uint inchannels, uint outchannels, uint inwidth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.KernelProduct1D(inchannels, outchannels, kwidth, transpose));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.KernelProduct1D(inchannels, outchannels, kwidth, transpose));
                }
            }
        ...
        public static void Convolution2D(uint inchannels, uint outchannels, uint inwidth, uint inheight,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Convolution2D(inchannels, outchannels, kwidth, kheight, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Convolution2D(inchannels, outchannels, kwidth, kheight, gradmode));
                }
            }
        ...
        public static void Deconvolution2D(uint inchannels, uint outchannels, uint inwidth, uint inheight,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Deconvolution2D(inchannels, outchannels, kwidth, kheight, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Deconvolution2D(inchannels, outchannels, kwidth, kheight, gradmode));
                }
            }
        ...
        public static void KernelProduct2D(uint inchannels, uint outchannels, uint inwidth, uint inheight,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.KernelProduct2D(inchannels, outchannels, kwidth, kheight, transpose));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.KernelProduct2D(inchannels, outchannels, kwidth, kheight, transpose));
                }
            }
        ...
        public static void Convolution3D(uint inchannels, uint outchannels, uint inwidth, uint inheight, uint indepth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Convolution3D(inchannels, outchannels, kwidth, kheight, kdepth, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Convolution3D(inchannels, outchannels, kwidth, kheight, kdepth, gradmode));
                }
            }
        ...
        public static void Deconvolution3D(uint inchannels, uint outchannels, uint inwidth, uint inheight, uint indepth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.Deconvolution3D(inchannels, outchannels, kwidth, kheight, kdepth, gradmode));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.Deconvolution3D(inchannels, outchannels, kwidth, kheight, kdepth, gradmode));
                }
            }
        ...
        public static void KernelProduct3D(uint inchannels, uint outchannels, uint inwidth, uint inheight, uint indepth,
            ...
            if (!shaders.ContainsKey(key)) {
                if (Environment.Precision == Environment.PrecisionMode.Float) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatPrecision.KernelProduct3D(inchannels, outchannels, kwidth, kheight, kdepth, transpose));
                }
                else if (Environment.Precision == Environment.PrecisionMode.FloatFloat) {
                    shaders.Add(key, new Shaders.Complex.Convolution.FloatFloatPrecision.KernelProduct3D(inchannels, outchannels, kwidth, kheight, kdepth, transpose));
                }
            }
