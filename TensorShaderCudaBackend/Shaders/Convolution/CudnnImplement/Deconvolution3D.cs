...
namespace TensorShaderCudaBackend.Shaders.Convolution.CudnnImplement {
    ...
    public sealed class Deconvolution3D : Shader {
        ...
        protected override void CheckArgument(params object[] args) {
            ...
            if (args[8] is not uint inwidth || !Limits.CheckWidth(inwidth)) {
                throw new ArgumentException(nameof(inwidth));
            }

            if (args[9] is not uint inheight || !Limits.CheckHeight(inheight)) {
                throw new ArgumentException(nameof(inheight));
            }

            if (args[10] is not uint indepth || !Limits.CheckHeight(indepth)) {
