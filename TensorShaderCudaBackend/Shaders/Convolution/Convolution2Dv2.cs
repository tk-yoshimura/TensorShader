...
namespace TensorShaderCudaBackend.Shaders.Convolution {
    ...
    public sealed class Convolution2Dv2 : Shader {
        ...
        /// <summary>1ブロックで実行する出力チャネル数</summary>
        private uint ThreadX { set; get; }

        /// <summary>1ブロックで実行するピクセル数</summary>
        public uint BatchPixels = 32;
        ...
        public Convolution2Dv2(uint inchannels, uint outchannels, uint kwidth, uint kheight) {
            ...
            uint thread_x = Kernel.DefaultBlockSize(OutChannels);
            uint smemlimits = Limits.MaxSharedMemoryLength;

            if(SharedMemoryUsage(inchannels, thread_x, BatchPixels, KernelWidth) > smemlimits) { 
                throw new ArgumentOutOfRangeException($"{nameof(inchannels)}, {nameof(outchannels)}, {nameof(kwidth)}");
            }
            ...
            this.ThreadX = thread_x;

            string code = $@"

            {Defines.FloatFloatFma}
            {Defines.StoreFloatSharedMemory(elemsize: 1, elements: InChannels * (BatchPixels + KernelWidth - 1), threads: ThreadX * BatchPixels, "us")}
            {Defines.StoreFloatSharedMemory(elemsize: 1, elements: InChannels * ThreadX, threads: ThreadX * BatchPixels, "vs")}

            __global__ void convolution_2dv2(const float* __restrict__ inmap, float* __restrict__ outmap, const float* __restrict__ filter,
                                             unsigned int oy_offset,
                                             unsigned int inwidth, unsigned int outwidth) {{

                unsigned int outch = {Defines.IndexX}, tidx = {Defines.ThreadIdX}, tidy = {Defines.ThreadIdY}, tid = tidx + {ThreadX} * tidy;
                unsigned int ox = {Defines.IndexY}, oy = oy_offset + {Defines.BlockIndexZ}, ix_offset = {Defines.BlockIndexX} * {BatchPixels};

                __shared__ float us[{InChannels * (BatchPixels + KernelWidth - 1)}];
                __shared__ float vs[{InChannels * ThreadX}];

                float uv_hi = 0.0, uv_lo = 0.0;

                for(unsigned int ky = 0, iy = oy; ky < {KernelHeight}; ky++, iy++){{

                    unsigned int inmap_idx = {InChannels} * (ix_offset + inwidth * iy);
                    store_smem_us(inmap + inmap_idx, us, tid);

                    { (KernelWidth <= 7 ? "#pragma unroll" : "") }
                    for(unsigned int kx = 0, ix = ox; kx < {KernelWidth}; kx++, ix++){{

                        unsigned int inmap_idx = {InChannels} * (ix_offset + inwidth * iy);
                        store_smem_vs(filter + , us, tid);

                        { (OutChannels % ThreadX != 0 ? $"if(outch < {OutChannels}){{" : "") }
        ...
        private static uint SharedMemoryUsage(uint inchannels, uint batch_outchannels, uint batchpixels, uint kernelwidth) { 
            uint kernelsize = inchannels * batch_outchannels;
            uint inmapsize = inchannels * (batchpixels + kernelwidth - 1);

            return kernelsize + inmapsize;
        }
    }
}
