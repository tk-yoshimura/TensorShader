...
namespace TensorShaderCudaBackend.Shaders.Pool {
    ...
    public sealed class MaxUnpool1D : Unpool1D {
        ...
        public MaxUnpool1D(uint channels, uint stride)
            ...
            __global__ void maxunpool_1d(const float* __restrict__ ingrad, const float* __restrict__ inpool, const float* __restrict__ inmap, float* __restrict__ outmap,
        ...
        protected override void CheckArgument(params object[] args) {
            ...
            if (args[0] is not CudaArray<float> ingrad || ingrad.Length < Channels * inwidth * batches) {
                throw new ArgumentException(nameof(ingrad));
            }

            if (args[1] is not CudaArray<float> inpool || inpool.Length < Channels * inwidth * batches) {
                throw new ArgumentException(nameof(inpool));
            }

            if (args[2] is not CudaArray<float> inmap || inmap.Length < Channels * outwidth * batches) {
                throw new ArgumentException(nameof(inmap));
            }

            if (args[3] is not CudaArray<float> outmap || outmap.Length < Channels * outwidth * batches) {
