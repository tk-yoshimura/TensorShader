...
namespace TensorShaderCudaBackend.Shaders.Trivector.Convolution.FloatFloatPrecision {
    ...
    public sealed class KernelProduct3D : Shader {
        ...
        public KernelProduct3D(uint inchannels, uint outchannels, uint kwidth, uint kheight, uint kdepth, bool transpose) {
            ...
            {Defines.FloatFloat.Trivector.KernelProd}
        ...
        public override void Execute(Stream stream, params object[] args) {
            ...
            CudaArray<float> dfloat_filter =
                WorkspaceReserver<float>.Request(stream, inmap.DeviceID, index: 0, InChannels * OutChannels * KernelWidth * KernelHeight * KernelDepth * 8);
        ...
        protected override void CheckArgument(params object[] args) {
            ...
            if (args[4] is not uint inwidth || !Limits.CheckWidth(inwidth, KernelWidth)) {
                throw new ArgumentException(nameof(inwidth));
            }

            if (args[5] is not uint inheight || !Limits.CheckHeight(inheight, KernelHeight)) {
                throw new ArgumentException(nameof(inheight));
            }

            if (args[6] is not uint indepth || !Limits.CheckDepth(indepth, KernelDepth)) {
            ...
            if (args[0] is not CudaArray<float> inmap || inmap.Length < InChannels * inwidth * inheight * indepth * batches * 3) {
                throw new ArgumentException(nameof(inmap));
            }

            if (args[1] is not CudaArray<float> outmap || outmap.Length < OutChannels * outwidth * outheight * outdepth * batches * 3) {
                throw new ArgumentException(nameof(outmap));
            }

            if (args[2] is not CudaArray<float> filter_value || filter_value.Length < InChannels * OutChannels * KernelWidth * KernelHeight * KernelDepth * 4) {
                throw new ArgumentException(nameof(filter_value));
            }

            if (args[3] is not CudaArray<float> filter_grad || filter_grad.Length < InChannels * OutChannels * KernelWidth * KernelHeight * KernelDepth * 4) {
