...
namespace TensorShaderCudaBackend.Shaders.Trivector.Convolution {
    ...
    public sealed class KernelProduct2D : Shader {
            ...
            __global__ void trivector_kernelproduct_2d(float3 *inmap, float3 *outmap, float4 *filter_value, float4 *filter_grad, 
                ...
                for(unsigned int ky = 0, iy = oy; ky < {KernelHeight}; ky++, iy++){{
                    for(unsigned int kx = 0; kx < {KernelWidth}; kx++){{
                        ...
                        if(inch < {InChannels} && outch < {OutChannels}){{
                            gq_hi.x += gq_hi.x;  gq_hi.y += gq_hi.y;  gq_hi.z += gq_hi.z;  gq_hi.w += gq_hi.w;
                            gq_lo.x += gq_lo.x;  gq_lo.y += gq_lo.y;  gq_lo.z += gq_lo.z;  gq_lo.w += gq_lo.w;
        ...
        public override void Execute(Stream stream, params object[] args) {
            ...
            hadd.Execute(stream, dfloat_filter, filter_grad, InChannels * OutChannels * KernelWidth * KernelHeight * 4);
        }
