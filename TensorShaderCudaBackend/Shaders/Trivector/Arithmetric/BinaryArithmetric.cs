namespace TensorShaderCudaBackend.Shaders.Trivector.Arithmetric {
    ...
    public sealed class BinaryArithmetric : Elementwise {
        ...
        public BinaryArithmetric(string name, string func)
            ...
            __global__ void {name}(const float3* __restrict__ inmap1, const float3* __restrict__ inmap2, float3* __restrict__ outmap, unsigned int n) {{
                unsigned int i = {Defines.IndexX};
                if (i >= n) {{
                    return;
                }}
                float3 y, x1 = inmap1[i], x2 = inmap2[i];
